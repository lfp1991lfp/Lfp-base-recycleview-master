...
//        recyclerView.setItemAnimator(new ScaleInRightAnimator(new OvershootInterpolator(20f)));
        recyclerView.setAdapter(new SnapHelperAdapter(this, R.layout.my_recyclerview_item,
                initData()));
...
//        snapHelper.setPageTransformer(new CustomTransformer());
    ...
    private List<SnapBean> initData() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            int imageId = mTypedArray.getResourceId(i, R.mipmap.m_1);//第一个参数为
            // ：所取图片在数组中的索引，第二个参数为：未找到时，返回的默认值id。
